...
namespace PredictiveKeyboard
{
    ...
    #region old_dic
        ...
        private void arpa_ngram(string text)
        {
            ...
            foreach (string ob in final_prob_smooth)
            {
                ...
                if (count <= windowSize)
                {
                    ...
                    try
                    {
                        output_string = SVM_log(count, output_string, svm_traing_path, data);
                    }
                    catch (Exception eee)
                    { }
                }
        ...
        private static string SVM_log(int count, string output_string, string svm_traing_path, string data)
        {
            ...
            output_string = count.ToString() + " qid:" + query_id.ToString() + " 1:" + ouput_temp[0].ToString() + " 2:" + ouput_temp[1].ToString() + " 3:" + frq + "\t\t\t# " + ouput_temp[4].ToString();
        ...
        private void setting_high_light(ArrayList arr_char,string last_char)
        {
            ...
            foreach (Control btn in panel12.Controls)
            {
                if (btn is Button)
                {
                    ...
                    if (btn.Text.IndexOf(last_char) >= 0)
                    {
                        new_str = (btn.Text).Substring(btn.Text.IndexOf(last_char) + 1);
                    }
                    else
                    {
                        new_str = btn.Text;
        ...
        private void Error_Handling(ref ArrayList final_prob_smooth, string typedString, string targetString, string frq)
        {
            ...
            ////////////////////////////////phnatic update
            //string st = str_obj;
            //string stt = Regex.Replace(st, "्", "");
            //stt = Regex.Replace(st, "म्", "ं");
            //stt = Regex.Replace(st, "न्", "ं");
            
            string stt = "";
            string inpp = "";

            string st = phonatic_redudency_remove(str_obj.Trim(), ref stt); // target string
            string inp = phonatic_redudency_remove(typedString.Trim(), ref inpp);    // typed string

            //string inp = typedString;
            //inpp = Regex.Replace(inp, "्", "");
            //inpp = Regex.Replace(st, "म्", "ं");
            //inpp = Regex.Replace(st, "न्", "ं"); 
            //inpp = removedupes(inpp);

            string Phonatic_text = "";
            //if (stt.Length >= inpp.Length)
            if (st.Length >= inp.Length)
            {
                //Phonatic_text = stt.Substring(0, inpp.Length);
                Phonatic_text = st.Substring(0, inp.Length);
            }
//            म्
//न्
//ं
            PScore = Math.Round(Cal_phonatic_method(inp, Phonatic_text), 3);
            //PScore = Math.Round(Cal_phonatic_method(inpp, Phonatic_text), 3);
            ///////////////phonatic update end
            ...
            //Score = (System.Convert.ToDouble(PScore) * lamda1 + System.Convert.ToDouble(strEDscr) * lamda2 + Math.Abs(System.Convert.ToDouble(StrWordFrequency) * lamda3)).ToString();
   ...
   #region Required proposed error tech
        ...
        private void SimulateForPKS()
        {
            ...
            toPrint = "\tTotal hit:\t" + count.ToString() + "\tTotal keypress:\t" + keypress.ToString() + "\tTotal Word=\t" + different_word.Length.ToString() + "\tPKS=\t" + PKS.ToString("00.0000");
